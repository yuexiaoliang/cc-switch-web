//! cc-switch-mini binary entry point.
//!
...
use cc_switch_mini_server::{cli, error, runtime};
use clap::Parser;
...
async fn main() -> std::process::ExitCode {
    ...
    let opts = match cli::Resolved::resolve(raw) {
        Ok(opts) => opts,
        Err(err) => {
            eprintln!("cc-switch-mini: failed to resolve options: {err}");
