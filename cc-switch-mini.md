...
**cc-switch-mini** 是 cc-switch 的**服务端/Web 衍生版本**，目标：
...
| 维度 | cc-switch（上游） | cc-switch-mini（本项目） |
| --- | --- | --- |
...
```
cc-switch-mini/                  # GitHub 仓库（fork 自 farion1231/cc-switch）
│
...
用户浏览器 ──HTTP──→ cc-switch-mini 进程
                        ...
                        │
                        ├── 上游 Database 层（复用）
                        │   └── SQLite（~/.local/share/cc-switch-mini/）
                        │
                        └── 系统配置文件写入
                            ├── ~/.codex/config.toml
                            ├── ~/.codex/auth.json
...
```bash
wget https://github.com/.../cc-switch-mini-linux-x64
chmod +x cc-switch-mini-linux-x64
sudo mv cc-switch-mini-linux-x64 /usr/local/bin/cc-switch-mini
```

**方式三：cargo install（有 Rust 环境）**

```bash
cargo install cc-switch-mini
```
...
```bash
# 自定义端口
cc-switch-mini --port 8080

# 数据目录
cc-switch-mini --data-dir /var/lib/cc-switch-mini
```
...
# 3. 产物：单个二进制文件
# target/release/cc-switch-mini-server
```
...
**版本号来源**：从上游 `package.json` 和 `Cargo.toml` 中读取，`cc-switch-mini` 不做独立版本号管理。
